...
如果电脑没有 Python，脚本会优先通过 Windows 自带的 `winget` 自动安装 Python 3.11；如果系统没有 `winget` 或 `winget` 安装失败，会自动改用 Python 官方安装包。安装过程需要能够访问 Python 软件源的网络，并可能被系统安全软件拦截。安装失败时窗口不会自动关闭，请保留窗口中的错误信息。
...
- 支持 1 至 70 号海域；11 至 70 号海域当前共用默认的 `10x10` 网格和潜艇配置 `(2, 2, 3, 4, 5)`。
...
- 11 至 70 关使用同一套默认潜艇长度，游戏活动变化后需要重新核对。
