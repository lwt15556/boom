...
class DiamondHitConfig:
    ...
    wreck_marker_component_ratio: float = 0.040
    wreck_marker_s_drop: float = 12.0
    strong_hit_score_threshold: float = 0.80
...
def classify_diamond_hit(
    ...
    strong_gray_hit = (
        score >= config.strong_hit_score_threshold
        and center_gray_ratio >= config.strong_hit_center_gray_ratio
        and component_ratio >= config.strong_hit_component_ratio
        and gray_excess >= config.strong_hit_gray_excess
    )

    if wreck_marker_visible or strong_gray_hit:
        state = "hit"
        confidence = max(score, 0.92)
    elif changed_ratio < config.min_changed_ratio:
        state = "unknown"
        confidence = 1.0 - changed_ratio / max(config.min_changed_ratio, 1e-6)
        confidence = max(0.0, min(1.0, confidence))
    elif score >= config.hit_score_threshold and passes_hit_gates(
        center_gray_ratio=center_gray_ratio,
...
def passes_hit_gates(
    *,
    ...
    """Require core wreck features before a high score can be treated as a hit."""
    return (
        center_gray_ratio >= config.min_center_gray_ratio
        and gray_excess >= config.min_gray_excess
        and component_ratio >= config.min_component_ratio
        and s_drop >= config.min_s_drop
    )


def passes_wreck_marker_gates(
    *,
    changed_ratio: float,
    center_gray_ratio: float,
    component_ratio: float,
    ...
    """Detect the clear gray wreck marker shown after a submarine is hit."""
    return (
        changed_ratio >= config.wreck_marker_changed_ratio
        and center_gray_ratio >= config.wreck_marker_center_gray_ratio
        and component_ratio >= config.wreck_marker_component_ratio
        and s_drop >= config.wreck_marker_s_drop
    )
